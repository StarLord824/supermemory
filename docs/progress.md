...
| `feat: container tag discovery + Home/Docs tabs` (on `master`) | Users no longer type a container tag blind. New `ops.listContainerTags()` derives the full tag set by paging through `POST /v3/documents/list` with **no** `containerTags` filter (which returns documents across every tag, each carrying its own `containerTags[]`) and deduping+counting — there is no native list-tags endpoint (`GET /v3/container-tags` 404s, confirmed live). Exposed three ways: a new `curator tags` CLI command (`src/cli-format.ts`'s pure `formatTagsTable`), a new `GET /api/tags` route, and a console `TagPicker` (native `<input list>` + `<datalist>` — suggests known tags as you type, still accepts free text for a fresh install with no data). Two new console tabs: **Home** (now the default — an overview dashboard reusing already-fetched stats: tags found, memory count for the active tag, Review support) and **Docs** (a static in-app CLI/MCP/tab reference). **Live-verified 2026-07-17** against the real server: `curator tags` and `GET /api/tags` both returned the real tag set (`curator_test`:2, `src_github`:12) identically; the three underlying facts (no native endpoint, unfiltered documents span tags, `{limit,page}` pagination) were each re-confirmed with direct requests; and a human browser walkthrough confirmed Home is the default tab showing live stats, the tag field suggests known tags while still accepting free text, no regression across Memories/Review/Forget/Graph, and the Docs tab renders — see `docs/api-verification.md` §15. Built via subagent-driven-development (7 tasks, fresh implementer + reviewer per task, zero Critical/Important findings across all per-task reviews). 11 new tests. |

**Total: 45+ commits; 166 passing tests across 17 test files; clean `tsc --noEmit` and `vite build`.**
...
- **Container tag discovery, live-verified 2026-07-17:** `curator tags` and the console's tag
  search+dropdown both list the real tags that exist (`curator_test`:2, `src_github`:12) instead of
  making the user guess a tag name. Derived by paging `POST /v3/documents/list` with no filter
  (no native list-tags endpoint exists — `GET /v3/container-tags` 404s). New Home tab (default,
  live overview stats) and Docs tab (in-app reference) both human-confirmed in the browser. See
  `docs/api-verification.md` §15.
